...
- **NEXT_TASK:** Wire `kegbot ideas` into the morning briefing as a weekly "what should I build this weekend?" section (Saturdays only). Or add a `forge save` command that persists ideas to a local JSON log so Kevin can revisit past suggestions. Could also wire `insights repos` into the summary dashboard so `kegbot insights` shows the full picture in one shot.
...
| 8   | `projects/idea-forge/forge.py` — AI weekend project idea generator. `forge trending` fetches live trending GitHub repos (Python/TypeScript/Go) via GitHub search API. `forge suggest` sends them to Claude with Kevin's profile and returns 3 opinionated weekend project ideas with names, concepts, stacks, and build plans. `insights repos` added to dev-insights — per-repo commit bar chart with velocity (commits/wk) and date range. `kegbot ideas` wired up as a delegate to forge.py. |
...
└── projects/
    ...
    └── idea-forge/              ← AI weekend project idea generator
        └── forge.py             ← trending, suggest commands
```
