...
- **NEXT_TASK:** Wire `kegbot forge ideas` into `briefing.py` as a weekly "project spark" section (once per week, suggest one idea from trending). Or: add `forge save` to persist a favorite idea to a local JSON file, and `forge list` to browse saved ideas. Could also explore giving the heatmap ASCII export or adding `insights repos --velocity` to show commit rate per day active.
...
| 8   | `projects/idea-forge/forge.py` — AI project idea generator. `forge trending` fetches GitHub Search API for repos gaining stars in Kevin's stack (TypeScript/Python/Go). `forge ideas` synthesizes 3 tailored project ideas via Claude. `forge plan` generates full weekend blueprints with day-by-day breakdowns and first commands. `forge stack` prints Kevin's profile. Also added `insights repos` to dev-insights — shows top repos by commit count with velocity indicators (🔥⚡▶). Live data: vball-tracker is Kevin's most active repo (15 commits). Both wired into `kegbot` as `kegbot forge` and `kegbot insights repos`. |
...
└── projects/
    ...
    └── idea-forge/              ← AI project idea generator
        ├── README.md
        └── forge.py             ← trending, ideas, plan, stack commands
```
