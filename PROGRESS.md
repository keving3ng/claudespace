...
## Status
- **RUN_COUNT:** 8
- **CURRENT_PHASE:** 5 — idea-forge built + dev-insights repos + briefing --activity
- **NEXT_TASK:** Expand `idea-forge` — add a `save` command to bookmark ideas to a local JSON file (`ideas.json`), so Kevin can keep a running list of things to build. Or build `kegbot digest` — a weekly summary command that rolls up: what was built in claudespace, top repos by activity, and a recap of the best recipe-ai suggestions or dev-insights data. Alternatively: add a `vball-tracker` integration since Kevin is actively building that — `kegbot vball` to show recent commits there.
...
| 8   | `projects/idea-forge/idea_forge.py` — AI project idea generator. Fetches trending GitHub repos by language stack, asks Claude to synthesize 5 tailored project ideas. Commands: `ideas` (5 ideas), `trending` (raw data), `spark` (one fully-pitched idea). Wired as `kegbot forge`. Also: added `insights repos` to dev-insights — per-repo commit breakdown + 14-day velocity bar chart (live: Kevin's hottest repo is `vball-tracker`, 22 commits, 15 in the last 14 days). Added `--activity` flag to briefing.py — includes per-repo breakdown in morning briefing context. |
...
└── projects/
    ...
    └── idea-forge/              ← AI project idea generator from GitHub trends
        ├── README.md
        └── idea_forge.py        ← ideas, trending, spark commands
```
