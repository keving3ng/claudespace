...
## Status
- **RUN_COUNT:** 8
- **CURRENT_PHASE:** 5 — idea-forge launched + dev-insights repos
- **NEXT_TASK:** Wire `kegbot insights repos` + `kegbot forge ideas` into the daily briefing (`--activity` flag that appends a short GitHub repo breakdown + one AI project idea to the morning briefing). Or: build `idea-forge`'s save/history feature — `forge save` bookmarks ideas you like to `ideas.json`, `forge history` lists them. Or: start `kevin-tools` — a unified launcher script that ties everything together with a single install.
...
| 8   | `insights repos` — added to dev-insights: per-repo commit breakdown with ASCII velocity bars and commits/wk metric. `projects/idea-forge/forge.py` — AI project idea generator: `forge trending` (GitHub search API, trending repos by language), `forge ideas` (trending → Claude → 3 tailored weekend project ideas), `forge spark` (pure creative generation, no trending context). Wired `kegbot forge` as a delegate. |
