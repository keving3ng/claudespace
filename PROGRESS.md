...
## Status
- **RUN_COUNT:** 8
- **CURRENT_PHASE:** 5 — idea-forge added + dev-insights repos command
- **NEXT_TASK:** Wire `insights repos` into `kegbot briefing` as an optional `--activity` flag (so the morning briefing can say "your most active repo today is vball-tracker"). Or: add a `forge save` command to persist interesting ideas to a local ideas.json for later review. Bonus: investigate `vball-tracker` — Kevin's been very active on it and it's not in our radar yet.
...
| 8   | `insights repos` — new subcommand showing per-repo commit breakdown with 7-day velocity indicator (↑ accelerating / ↓ cooling). Live data: `vball-tracker` is Kevin's most active repo (15 commits — not previously known!). `projects/idea-forge/forge.py` — AI project idea generator: `forge suggest` fetches trending GitHub repos per language + Claude synthesis; `forge inspire` pure AI brainstorm from Kevin's profile. Wired as `kegbot forge`. Updated ABOUT_KEVIN.md with vball-tracker discovery. |
...
└── projects/
    ...
    └── idea-forge/              ← AI project idea generator
        └── forge.py             ← suggest (trending repos), inspire (pure AI)
```
